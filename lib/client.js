...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var CSS = [
      ...
      '.rdr-treeDel{flex:none;width:18px;height:18px;border:none;background:transparent;color:var(--dsw-alias-label-secondary);border-radius:50%;cursor:pointer;font-size:12px;line-height:1;opacity:0;display:inline-flex;align-items:center;justify-content:center;padding:0}',
      '.rdr-treeRow:hover .rdr-treeDel{opacity:1}',
      '.rdr-treeDel:hover{color:var(--dsw-alias-state-error-primary,#e5484d);background:var(--dsw-alias-bg-layer-2)}',
    ].join('')
    ...
    var API = {
      ...
      localScan: API_BASE + '/local/scan',
      localRead: API_BASE + '/local/read',
    }
    ...
    async function showShelf() {
      ...
      if (books.length === 0) {
        addAssistantMessage('书架还是空的。输入书名即可从在线书源搜索，或把 TXT 放进 ' + state.localDir + ' 目录（输入「本地书」扫描）。', true)
        return
      }
    ...
    /** 扫描本地书库目录并展示本地书。 */
    async function showLocalShelf() {
      var loading = addLoadingMessage()
      try {
        var settings = loadSettings()
        var dirQuery = settings.localDir ? '?dir=' + encodeURIComponent(settings.localDir) : ''
        var data = await apiGet(API.localScan + dirQuery)
        state.localDir = data.dir
        loading.remove()
        if (data.books.length === 0) {
          addAssistantMessage('本地书库为空。把 .txt 文件放到：' + data.dir + '，然后输入「本地书」。', true)
          return
        }
        var rows = data.books.map(function (b) {
          var sizeLabel = b.size > 1048576 ? (b.size / 1048576).toFixed(1) + ' MB' : (b.size / 1024).toFixed(0) + ' KB'
          return makeCardRow(
            '<div class="rdr-cardTitle">' + escapeHtml(b.name) + '</div><div class="rdr-cardSub">' + escapeHtml(b.path) + '</div>',
            sizeLabel,
            function () { openLocalBook(b) },
          )
        })
        addCardMessage('本地书（' + data.books.length + ' 本，目录：' + data.dir + '）：', rows)
      } catch (error) {
        loading.remove()
        addAssistantMessage('本地书库扫描失败：' + error.message, true)
      }
    }

    /** 打开本地书：读取 → 拆分章节 → 入库 → 开始阅读。 */
    async function openLocalBook(file) {
      var loading = addLoadingMessage()
      try {
        var data = await apiPost(API.localRead, { path: file.path })
        loading.remove()
        if (data.chapters.length === 0) {
          addAssistantMessage('《' + file.name + '》没有解析到章节。', true)
          return
        }
        var book = {
          id: 'local|' + file.path,
          ...
          url: file.path,
          localPath: file.path,
          chapters: data.chapters.map(function (c, i) { return { index: i, title: c.title, url: '', text: c.text } }),
          chapterIndex: 0,
          savedAt: Date.now(),
        }
        await dbPut('books', book)
        state.current = book
        setHeader(book.title, '本地书 · ' + data.chapterCount + ' 章')
        addAssistantMessage('《' + book.title + '》已加入书架，开始阅读（输入「下一章」继续）。', true)
        notifyTree()
        await readChapter(0)
      } catch (error) {
        loading.remove()
        addAssistantMessage('打开本地书失败：' + error.message, true)
      }
    }
    ...
    function showHelp() {
      addAssistantMessage(
        ...
        '· 第N章 → 跳转章节\n' +
        '· 目录 → 章节列表\n' +
        '· 书架 → 已保存的书（侧边栏每本书有 × 可删除）\n' +
        '· 换源 → 用当前书名在所有书源重新搜索（可「换源 关键字」）\n' +
        '· 本地书 → 扫描本地书库（默认 ~/dsh-reader-books/，可用「设置 书库目录 路径」改）\n' +
        '· 下载 → 整本下载到本地（TXT）\n' +
        '· 书源 → 内置书源列表\n' +
    ...
    function applySettingCommand(text) {
      ...
      if (key === '书库目录' || key === '本地目录') {
        if (value === '') {
          addAssistantMessage('用法：设置 书库目录 <路径>，例如：设置 书库目录 D:\\小说', true)
          return true
        }
        saveSettings({ localDir: value })
        addAssistantMessage('本地书库目录已设为：' + value + '（输入「本地书」重新扫描）', true)
        return true
      }
      addAssistantMessage('支持：设置 字号 大/小/数字、设置 行距 数字、设置 字体 宋体/楷体/黑体/微软雅黑/苹方/等宽、设置 书库目录 <路径>。', true)
      return true
    }
    ...
    /** 换源：用当前书名（或指定关键字）在所有书源重新搜索。 */
    function changeSource(keyword) {
      var book = state.current
      if (book === null) {
        addAssistantMessage('还没有打开书。输入「换源 关键字」可直接搜索。', true)
        return
      }
      var kw = keyword !== '' ? keyword : book.title
      addAssistantMessage('正在所有书源重新搜索《' + kw + '》，点击其他来源的书籍即可换源。', true)
      searchBooks(kw)
    }
    ...
    function handleInput() {
      ...
      if (jump !== null) {
        var n = Number(jump[1])
        var book = state.current
        if (book === null) { addAssistantMessage('还没有打开书。', true); return }
        // 优先：按章节标题里的数字匹配（适配目录前面有非正文章节的情况）
        var matchIdx = -1
        for (var ci = 0; ci < book.chapters.length; ci++) {
          var cm = /^\s*(?:第)?(\d+)[.章、\s]/.exec(book.chapters[ci].title)
          if (cm !== null && Number(cm[1]) === n) { matchIdx = ci; break }
        }
        if (matchIdx >= 0) {
          jumpChapter(matchIdx)
        } else {
          // 兜底：按数组序号（第1项 = 输入"第1章"）
          jumpChapter(n - 1)
        }
      ...
      if (/^(本地书|本地|localfiles)$/.test(lower)) { showLocalShelf(); return }
      if (/^(下载|保存|存书)$/.test(lower)) { startDownload(); return }
      if (/^(换源|切换书源|换书源)$/.test(lower)) { changeSource(''); return }
      var sourceKw = /^换源s+(.+)$/.exec(text)
      if (sourceKw !== null) { changeSource(sourceKw[1].trim()); return }
    ...
    function treeRow(icon, title, meta, current, onClick, onDelete) {
      var row = document.createElement('div')
